...
<div class="container my-4">
    <h1 class="text-center mb-4">size List</h1>
 <div class="col-md-8">
            <input type="text" id="searchInput" class="form-control" placeholder="Search by size">
        </div>
    <!-- Search Bar and Add size Button -->
    <div class="row mb-3">
        <div class="col-md-4 text-end">
            <a href="{{ route('size.create') }}" class="btn btn-success">Add size</a>
        </div>
    </div>
        ...
        function fetchProducts(search = '', page = 1) {
            $.ajax({
                ...
                success: function (response) {
                    const tableBody = $('#sizeTable tbody');
                    ...
                    response.data.forEach((size) => {
                        tableBody.append(`
                            <tr>
                                ...
                                <td>${size.status}</td>
                                <td>
                                    <a href="/admin/size/edit/${size.id}" class="btn btn-sm btn-warning">Edit</a>
                                    <a href="/admin/size/delete/${size.id}" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')">Delete</a>
                                </td>
                            </tr>
                        `);
                    });
